...
def main(region: str = None):
    ...
    Args:
        region: If given, overrides ExperimentConfig's default region
            (previously this function always trained on config.region's
            default, "Sul", regardless of any --region CLI flag).
    """
    # =====================================================================
    # INITIALIZATION
    # =====================================================================
    ...
    if out["valid_mask"] is not None:
        ...
        if zero_valid.sum() == 0 and zero_pixels.sum() > 0:
            ...
            logger.warning("   The autoencoder will NEVER see non-drought examples.")
            logger.warning("   This may affect the quality of learned representations.")
