...
Examples:
  ...
  # Run inference (RECOMMENDED): reuses the threshold already fixed on
  # validation during grid search — never touches test data to pick it
  python main.py inference --use-calibration --optimization-metric mcc

  # Inference without calibration
  python main.py inference --p 9 --q 1 --model-type pretrained --no-calibration

  # Force a fresh threshold recomputation from validation data (still never touches test)
  python main.py inference --recalibrate --optimization-metric mcc
        """
    )
    ...
    inf_parser.add_argument('--use-validation-fallback', action='store_true', default=False,
